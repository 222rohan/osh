...
void copy_directory(const char *source_dir, const char *dest_dir, int force, int verbose) {
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (entry->d_type == DT_DIR) {
            ...
            delete_directory(source_path, verbose);
        }
        else if (entry->d_type == DT_REG) {
            copy_file(source_path, dest_path, force, verbose);
            delete_file(source_path, verbose);
        }
    }
...
void print_usage(const char *program_name) {
    ...
    printf("Options:\n");
    printf("  -r, --recursive    Move directories recursively\n");
...
int main(int argc, char *argv[]) {
    ...
    if (S_ISDIR(statbuf.st_mode)) {
        ...
        delete_directory(source, verbose);
    }
    else if (S_ISREG(statbuf.st_mode)) {
        copy_file(source, destination, force, verbose);
        delete_file(source, verbose);
    }
    else {
    ...
    printf("Successfully moved from %s to %s\n", source, destination);
    return 0;
}
